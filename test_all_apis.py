...
class TestAdminEndpoints:
    ...
    def test_deactivate_user(self, access_token_admin):
        ...
        create_response = client.post("/api/v1/auth/register", json={
            "email": "deactivate@test.com",
            "phone": "+998901234571",
            "full_name": "Deactivate User",
            "password": "test1234"
        })
        user_id = 2  # ID yaratish asosida

        response = client.put(f"/api/v1/admin/users/{user_id}/deactivate", headers=headers)
        assert response.status_code == 200
